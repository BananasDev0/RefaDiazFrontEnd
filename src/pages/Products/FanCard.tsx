...
const FanCard = ({ product }: FanCardProps) => {
  ...
  const handleEdit = (event: MouseEvent<HTMLElement>) => {
    ...
    if (product.id) {
      navigate(`/products/abanicos/edit/${product.id}`);
    }
  ...
  return (
    <>
      <Card
        ...
        onClick={() => product.id && navigate(`/products/abanicos/edit/${product.id}`)}
      >
        <CardHeader
          title={(
          ...
          action={(
            <IconButton aria-label="acciones del abanico" onClick={handleMenuClick}>
              <MoreVertIcon />
            </IconButton>
          )}
      ...
      <ConfirmDialog
        ...
        description={`¿Estás seguro de que quieres eliminar el abanico "${product.name}"? Esta acción no se puede deshacer.`}
